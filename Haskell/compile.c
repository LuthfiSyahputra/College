...
int tempcompile(int argc, char* argv[]) {
    ...
    // main executable
    systemf_s("ghc %s.c -o %s/%s", argv[0], dir, argv[0]);
...
int compile(int argc, char* argv[], char* filename, char* output_dir){
    ...
    // main compile
    if(cmdbool[ASM_FILE] == TRUE) systemf("ghc %s -O%d -S -masm=intel %s.hs -o %s_O%d.s", exParams, optimize, filename, outputfile, optimize);
    if(cmdbool[EXE_FILE] == TRUE) systemf("ghc %s -O%d %s.hs -o %s_O%d", exParams, optimize, filename, outputfile, optimize);
    if(cmdbool[DEBUG] == TRUE) {
        printf("%s\n\n%47s\n\n%s\n", __border__, "DEBUGGING", __border__);
        systemf("ghc %s -O%d %s.hs -o %s_O%d", exParams, optimize, filename, outputfile, optimize);
